...
const CheckoutForm = () => {
  ...
  const createPaymentIntent = async () => {
    try {
      const { data } = await axios.post(
        ...
        JSON.stringify({ cartItems, calculateTotal })
      );
  ...
  return (
    <div
      ...
      {succeded ? (
        <article className={succeded ? "" : "mb-4"}>
          <h4>Thank you</h4>
        ...
        <article className='mb-4'>
          <h4 className='font-bold'>Hello, {user && user.name}</h4>
          <p className='font-semibold'>Your total is: ${calculateTotal()}</p>
      ...
      <form id='payment-form' onSubmit={handleSubmit}>
        ...
        {/* Error when processing the payment */}
        {error && (
